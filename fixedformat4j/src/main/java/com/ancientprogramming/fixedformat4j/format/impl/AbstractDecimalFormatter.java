 ...
 * @since 1.0.0
 */
public abstract class AbstractDecimalFormatter<T extends Number> extends
  ...
  public String asString(T obj, FormatInstructions instructions) {
    ...
    String afterDelimiter = rawString
        .substring(rawString.indexOf(decimalSeparator) + 1, rawString.length());
    if (LOG.isDebugEnabled()) {
    ...
    String delimiter =
        useDecimalDelimiter ? "" + instructions.getFixedFormatDecimalData().getDecimalDelimiter()
            : "";
